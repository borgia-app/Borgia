...
from api.views import AuthGenerateJWT, AuthVerifyJWT
...
urlpatterns = [
    ...
    url(r'^(?P<group_name>[\w-]+)/shared_events/(?P<pk>\d+)/participants/(?P<participant_pk>(\w|\d)+)/$',
        SharedEventRmParticipant.as_view(),
        name='url_sharedevent_rm_participant'),
    url(r'^(?P<group_name>[\w-]+)/shared_events/(?P<pk>\d+)/registered/(?P<registered_pk>(\w|\d)+)/$',
        SharedEventRmRegistered.as_view(),
        name='url_sharedevent_rm_registered'),
