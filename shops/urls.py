...
shops_patterns = [
    ...
    path('<str:group_name>/products/', include([
        ...
        path('<int:pk>/', include([
            path('', ProductRetrieve.as_view(), name='url_product_retrieve'),
            path('update/', ProductUpdate.as_view(), name='url_product_update'),
            ...
            path('remove/', ProductRemove.as_view(), name='url_product_remove'),
        ]))
    ]))
]
